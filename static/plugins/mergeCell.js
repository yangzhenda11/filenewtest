 ...
 * @param {}
 *            _w_table_colnum 为需要合并单元格的所在列。为数字或者数字的数组，从最左边第一列为1开始算起。
 * 
 */
function table_rowspan(_w_table_id, _w_table_colnum){
	if(typeof _w_table_colnum == 'number'){
		_w_table_rowspan(_w_table_id,_w_table_colnum);
	}else if(typeof _w_table_colnum == 'object'){
		$.each(_w_table_colnum, function(index,_w_table_colnum_item) {
			_w_table_rowspan(_w_table_id,_w_table_colnum_item);
		});
	}
}
	...
	_w_table_Obj.each(function(i) {
		if (i == 0) {
			...
			if (_w_table_firsttd.text() == _w_table_currenttd.text()) {
                ...
                _w_table_firsttd.css('vertical-align','middle');//垂直居中
			} else {
				_w_table_firsttd = $(this);
				_w_table_SpanNum = 1;
			}
 ...
 *            如果为数字，则从最左边第一行为1开始算起。"even" 表示偶数行, "odd" 表示奇数行 "3n+1",
 *            表示的行数为1、4、7、10.......
 * @param {}
